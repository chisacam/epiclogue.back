...
export const search = async (req, res, next) => {
  ...
  if (searchType === 'Board') {
    // 글 제목으로 검색
    try {
      searchResult = await boardDAO.searchByTitleOrTag(queryString, latestId, category, requestSize)
    } catch (e) {
    ...
    try {
      searchResult =
        queryString[0] === '@'
          ? await userDAO.searchByScreenIdOrNickname(queryString.substr(1), latestId, requestSize)
          : await userDAO.searchByScreenIdOrNickname(queryString, latestId, requestSize)
    } catch (e) {
